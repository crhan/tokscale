...
pub fn render(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    let widths = if is_very_narrow {
        ...
        // Date is now "%m-%d" (5 chars), so it needs far less than the old
        // 30 %; the reclaimed width goes to Tokens and Cost.
        vec![
...
mod tests {
    ...
    fn full_mode_drops_year_when_layout_does_not_fit() {
        // 110 cols is full mode (>= 80) but narrower than the ~112-col full
