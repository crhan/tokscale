...
fn render_table(frame: &mut Frame, app: &mut App, area: Rect) {
    ...
    // sync. Trade-off: at the very bottom a page spanning ≥2 days can leave the
    // last row one line short until the next keypress — acceptable since hourly
    // defaults to newest-first, so the bottom (oldest) is rarely visited.
    ...
    app.max_visible_items = data_rows_shown.max(1);
